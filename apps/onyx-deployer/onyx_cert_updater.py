...
class OnyxCertUpdater:
    ...
    def login(self):
        ...
        login_data = urllib.parse.urlencode(
            {"f_user_id": self.username, "f_password": self.password}
        ).encode("utf-8")
    ...
    def execute_command(self, cmd):
        ...
        try:
            ...
            if result.get("status") == "OK":
                ...
                self.logger.error(
                    f"Command failed: {result.get('status_message', 'Unknown error')}"
                )
                return result
    ...
    def get_cert_validity(self, cert_name):
        ...
        """
        result = self.execute_command("show crypto certificate")
        if not result or result.get("status") != "OK":
            return None
...
def main():
    ...
    parser.add_argument("--hostname", required=True, help="Onyx switch hostname or IP address")
    parser.add_argument("--username", required=True, help="Switch username with admin access")
    ...
    parser.add_argument("--key-file", required=True, help="X.509 Private key filename (PEM format)")
    parser.add_argument(
        "--cert-file", required=True, help="X.509 Certificate filename (PEM format)"
    )
    parser.add_argument(
        "--force-update", action="store_true", help="Force update even if certificate seems current"
    )
    parser.add_argument(
        "--no-save", action="store_true", help="Do not save configuration after update"
    )
    ...
    updater = OnyxCertUpdater(
        hostname=args.hostname, username=args.username, password=args.password, debug=args.debug
    )
    ...
    # Save configuration
    if not args.no_save and not updater.save_config():
